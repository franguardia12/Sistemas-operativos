...
{
	if (arg[PRIMER_ARG] != DOLAR) {
		return arg;
	}

	char *nombre_var = arg + 1;

	if (nombre_var[PRIMER_ARG] == INTERROGACION &&
	    nombre_var[SEGUNDO_ARG] == END_STRING) {
		arg[PRIMER_ARG] = CARACTER_CERO + status;
		arg[SEGUNDO_ARG] = END_STRING;
		return arg;
	}

	char *variable = getenv(nombre_var);
	if (variable != NULL) {
		if (strlen(variable) > strlen(arg)) {
			arg = realloc(arg, strlen(variable) + 1);
		}
		strcpy(arg, variable);
	} else {
		arg[PRIMER_ARG] = END_STRING;
	}
